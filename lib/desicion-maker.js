...
const isDuplicate = metaData => {
  var duplicates = registeredUnWatchedShows.find(function(showMetaData){
    if(showMetaData.season === metaData.season && showMetaData.episode === metaData.episode && showMetaData.title === metaData.title) {
      return true
    }
  })
  if(duplicates) {
...
async function decide(rssPost) {
  ...
  let [watched, downloaded] = await Promise.all([isWatched(metaData), isDownloaded(metaData)])
